...
public class GesturePane extends Pane {
	...
	public GesturePane(Parent target, ScrollMode mode) {
		...
		setOnScroll(e -> {
			...
			case ZOOM:
				...
				if (currentScale.isEqualTo(oldScale, 0.01).get()) return; // no
																			// delta
				double delta = 1 + (currentScale.subtract(oldScale).get());
				double scale = affine.getMxx() * delta;
				if (scale <= minScale.get() || scale >= maxScale.get()) return; // out
																				// of
																				// limit
	...
	private void animateValue(double from, double to, Duration duration, DoubleConsumer consumer,
			Runnable callback) {
		timeline.stop();
		...
		}, to, new DecelerateInterpolator(2f));
