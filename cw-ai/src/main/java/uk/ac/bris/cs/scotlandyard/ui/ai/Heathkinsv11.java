	...
	private static class MyAI implements Player {
        ...
        private Set<DataNode> nextNodes(Set<DataNode> moves, Graph<Integer, Transport> graph, int future) {
                ...
                for(DataNode move : moves){
                    Set<List<PlayerData>> nextPlayerData = new HashSet<>();
                    nextPlayerData.add(move.playerList());
                    int i = 0;
                    for(PlayerData player : move.playerList()){
                            //Tmp storage set
                            Set<List<PlayerData>> tmp = new HashSet<>();
                        ...
                        nextPlayerData.clear();
                        nextPlayerData.addAll(tmp);
                        i++;
                    }    
                }
            return nextNodes;
